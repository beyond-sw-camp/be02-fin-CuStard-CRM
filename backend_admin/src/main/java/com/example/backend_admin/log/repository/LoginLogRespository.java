...
public interface LoginLogRespository extends JpaRepository<LoginLog, Long> {
    ...
    @Query(value = "SELECT COUNT(*) AS two_days_ago FROM login_log WHERE login_log.created_date BETWEEN DATE_SUB(CURRENT_DATE()-2, INTERVAL 90 DAY) AND DATE_SUB(CURRENT_DATE()-2) GROUP BY login_log.created_date", nativeQuery = true)
    Long findByTwoDaysAgoActiveCount();

    @Query(value = "SELECT COUNT(*) AS one_days_ago FROM login_log WHERE login_log.created_date BETWEEN DATE_SUB(CURRENT_DATE()-1, INTERVAL 90 DAY) AND DATE_SUB(CURRENT_DATE()-1) GROUP BY login_log.created_date", nativeQuery = true)
    ...
    @Query(value = "SELECT T.time_table, IFNULL(login_time_table.login_count, 0) AS login_count FROM (SELECT 0 AS time_table UNION SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7 UNION SELECT 8 UNION SELECT 9 UNION SELECT 10 UNION SELECT 11 UNION SELECT 12 UNION SELECT 13 UNION SELECT 14 UNION SELECT 15 UNION SELECT 16 UNION SELECT 17 UNION SELECT 18 UNION SELECT 19 UNION SELECT 20 UNION SELECT 21 UNION SELECT 22 UNION SELECT 23) AS T LEFT JOIN" +
            " ( SELECT DATE_FORMAT(created_date ,'%H') AS login_time, IFNULL(COUNT(*) , 0) AS login_count FROM (SELECT * FROM pampam.login_log WHERE created_date BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 2 DAY) AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)) AS login_time_table GROUP BY DATE_FORMAT(created_date ,'%H')) AS login_time_table ON T.time_table = login_time_table.login_time", nativeQuery = true)
