...
public interface LoginLogRespository extends JpaRepository<LoginLog, Long> {
    ...
    @Query(value = "SELECT COUNT(*) AS two_days_ago FROM login_log WHERE login_log.created_date < DATE_SUB(NOW()-24, INTERVAL 90 DAY)", nativeQuery = true)
    Long findByTwoDaysAgoDormancyCount();

    @Query(value = "SELECT COUNT(*) AS one_days_ago FROM login_log WHERE login_log.created_date < DATE_SUB(NOW(), INTERVAL 90 DAY)", nativeQuery = true)
    Long findByOneDaysAgoDormancyCount();
